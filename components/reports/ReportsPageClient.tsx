...
export function ReportsPageClient({
  ...
  const {
    ...
    initialData: scope === initialSummary.scope ? initialSummary : undefined,
  ...
  const { data: settingsData } = useQuery({
    ...
    initialData: { gym_name: initialGymName },
    staleTime: 5 * 60_000,
    gcTime: 30 * 60_000,
  });
  ...
  const exportPdf = useCallback(async () => {
    ...
    autoTable(doc, {
      ...
      styles: { font: tableFont, fontSize: 9, cellPadding: PDF_CELL_PAD, textColor: [0, 0, 0], fillColor: [255, 255, 255] },
      headStyles: { font: tableFont, fontStyle: "bold", fontSize: 9, fillColor: PDF_HEADER_RGB, textColor: 255, cellPadding: PDF_CELL_PAD },
      columnStyles: { 0: { cellWidth: 300, halign: "left" }, 1: { cellWidth: 215, halign: "right" } },
      didParseCell: (hook) => {
        stripeBodyRow(hook);
        if (hook.section === "head") hook.cell.styles.halign = hook.column.index === 0 ? "left" : "right";
        if (hook.section === "body") hook.cell.styles.halign = hook.column.index === 0 ? "left" : "right";
      },
      tableWidth: PDF_CONTENT_W,
      margin: { left: margin, right: margin },
    });
    let yAfter = (doc as unknown as { lastAutoTable: { finalY: number } }).lastAutoTable.finalY + 20;
    autoTable(doc, {
      startY: yAfter,
      theme: "plain",
    ...
    <div className="mx-auto w-full max-w-6xl p-4 md:p-6">
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-end">
        <div className="space-y-1">
          <label className="text-xs font-medium text-zinc-600" htmlFor="scope">Period</label>
          <select
            id="scope"
                  ...
                  <tbody>
                    {data.payments.map((p) => (
                      <tr key={p.id} className="border-b border-zinc-100">
                        ...
                        <td className="py-2 pr-2 text-zinc-600">{formatDateShortIST(p.payment_date)}</td>
      ...
      {isFetching && data ? <div className="mt-2 text-xs text-zinc-500">Refreshing report…</div> : null}
    </div>
  );
}
